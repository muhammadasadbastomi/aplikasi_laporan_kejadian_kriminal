...
<div class="row">
    <div class="col-12">
        <div class="card">
            <form class="floating-labels m-t-40" action="{{route('admin.gangguan.update',$gangguan->id)}}" method="POST"
                enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="card-body">
                    <div class="form-group m-b-40">
                        <input type="text" name="no_gangguan" value="{{$gangguan->no_gangguan}}" class="form-control"
                            id="input1" required>
                        <span class="bar"></span>
                        <label for="input1">Nomor Gangguan</label>
                    </div>
                    <div class="form-group m-b-40">
                        <select class="form-control p-0" name="camat_id" id="input6" required>
                            <option>Pilih Camat Penanggung Jawab</option>
                            @foreach ($camat as $d)
                            <option value="{{$d->id}}" {{$d->id == $gangguan->camat_id ? 'selected' : ''}}>
                                {{$d->nip . ' - ' . $d->nama }}</option>
                            @endforeach
                    ...
                    <div class="form-group m-b-40">
                        <select class="form-control p-0" name="kasi_id" id="input6" required>
                            ...
                            @foreach ($kasi as $d)
                            <option value="{{$d->id}}" {{$d->id == $gangguan->kasi_id ? 'selected' : ''}}>
                                {{$d->nip . ' - ' . $d->nama }}</option>
                            @endforeach
                        </select><span class="bar"></span>
                    ...
                    <div class="form-group m-b-40">
                        <select class="form-control p-0" name="petugas_id" id="input6" required>
                            ...
                            <option value="{{$d->id}}" {{$d->id == $gangguan->petugas_id ? 'selected' : ''}}>
                    ...
                    <div class="form-group m-b-40">
                        <select class="form-control p-0" name="desa_id" id="input6" required>
                            ...
                            @foreach ($desa as $d)
                            <option value="{{$d->id}}" {{$d->id == $gangguan->desa_id ? 'selected' : ''}}>
                    ...
                    <div class="form-group m-b-40">
                        <input type="date" name="tanggal_gangguan" value="{{$gangguan->tanggal_gangguan}}"
                    ...
                    <div class="form-group m-b-5">
                        <textarea class="form-control" name="deskripsi_gangguan" rows="4" id="input7"
                            required>{{$gangguan->deskripsi_gangguan}}</textarea>
                        <span class="bar"></span>
                        <label for="input7">Deskripsi Gangguan</label>
                    </div>
                    <div class="form-group m-b-40">
                        <input type="text" name="penanganan" value="{{$gangguan->penanganan}}" class="form-control"
                            id="input1" required>
                        <span class="bar"></span>
